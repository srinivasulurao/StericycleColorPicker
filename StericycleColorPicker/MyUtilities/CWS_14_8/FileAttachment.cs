namespace MyUtilities.CWS_14_8
{
    using System;
    using System.CodeDom.Compiler;
    using System.ComponentModel;
    using System.Diagnostics;
    using System.Threading;
    using System.Xml.Serialization;

    [Serializable, XmlInclude(typeof(FileAttachmentIncident)), XmlType(Namespace="urn:objects.ws.rightnow.com/v1_2"), XmlInclude(typeof(SharedFileAttachmentIncident)), DesignerCategory("code"), XmlInclude(typeof(SharedFileAttachmentShared)), XmlInclude(typeof(FileAttachmentCommon)), XmlInclude(typeof(SharedFileAttachmentCommon)), XmlInclude(typeof(FileAttachmentShared)), XmlInclude(typeof(FileAttachmentAnswer)), XmlInclude(typeof(SharedFileAttachmentAnswer)), GeneratedCode("System.Xml", "4.0.30319.18060"), DebuggerStepThrough]
    public class FileAttachment : INotifyPropertyChanged
    {
        private ActionEnum actionField;
        private bool actionFieldSpecified;
        private string contentTypeField;
        private DateTime createdTimeField;
        private bool createdTimeFieldSpecified;
        private byte[] dataField;
        private string fileNameField;
        private MyUtilities.CWS_14_8.ID idField;
        private int sizeField;
        private bool sizeFieldSpecified;
        private DateTime? updatedTimeField;
        private bool updatedTimeFieldSpecified;

        public event PropertyChangedEventHandler PropertyChanged;

        protected void RaisePropertyChanged(string propertyName)
        {
            PropertyChangedEventHandler propertyChanged = this.PropertyChanged;
            if (propertyChanged != null)
            {
                propertyChanged(this, new PropertyChangedEventArgs(propertyName));
            }
        }

        [XmlAttribute]
        public ActionEnum action
        {
            get
            {
                return this.actionField;
            }
            set
            {
                this.actionField = value;
                this.RaisePropertyChanged("action");
            }
        }

        [XmlIgnore]
        public bool actionSpecified
        {
            get
            {
                return this.actionFieldSpecified;
            }
            set
            {
                this.actionFieldSpecified = value;
                this.RaisePropertyChanged("actionSpecified");
            }
        }

        [XmlElement(Order=0)]
        public string ContentType
        {
            get
            {
                return this.contentTypeField;
            }
            set
            {
                this.contentTypeField = value;
                this.RaisePropertyChanged("ContentType");
            }
        }

        [XmlElement(Order=1)]
        public DateTime CreatedTime
        {
            get
            {
                return this.createdTimeField;
            }
            set
            {
                this.createdTimeField = value;
                this.RaisePropertyChanged("CreatedTime");
            }
        }

        [XmlIgnore]
        public bool CreatedTimeSpecified
        {
            get
            {
                return this.createdTimeFieldSpecified;
            }
            set
            {
                this.createdTimeFieldSpecified = value;
                this.RaisePropertyChanged("CreatedTimeSpecified");
            }
        }

        [XmlElement(DataType="base64Binary", Order=2)]
        public byte[] Data
        {
            get
            {
                return this.dataField;
            }
            set
            {
                this.dataField = value;
                this.RaisePropertyChanged("Data");
            }
        }

        [XmlElement(Order=3)]
        public string FileName
        {
            get
            {
                return this.fileNameField;
            }
            set
            {
                this.fileNameField = value;
                this.RaisePropertyChanged("FileName");
            }
        }

        [XmlElement(Order=4)]
        public MyUtilities.CWS_14_8.ID ID
        {
            get
            {
                return this.idField;
            }
            set
            {
                this.idField = value;
                this.RaisePropertyChanged("ID");
            }
        }

        [XmlElement(Order=5)]
        public int Size
        {
            get
            {
                return this.sizeField;
            }
            set
            {
                this.sizeField = value;
                this.RaisePropertyChanged("Size");
            }
        }

        [XmlIgnore]
        public bool SizeSpecified
        {
            get
            {
                return this.sizeFieldSpecified;
            }
            set
            {
                this.sizeFieldSpecified = value;
                this.RaisePropertyChanged("SizeSpecified");
            }
        }

        [XmlElement(IsNullable=true, Order=6)]
        public DateTime? UpdatedTime
        {
            get
            {
                return this.updatedTimeField;
            }
            set
            {
                this.updatedTimeField = value;
                this.RaisePropertyChanged("UpdatedTime");
            }
        }

        [XmlIgnore]
        public bool UpdatedTimeSpecified
        {
            get
            {
                return this.updatedTimeFieldSpecified;
            }
            set
            {
                this.updatedTimeFieldSpecified = value;
                this.RaisePropertyChanged("UpdatedTimeSpecified");
            }
        }
    }
}

